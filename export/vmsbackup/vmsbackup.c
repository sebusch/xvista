...
int 	cflag, dflag, eflag, sflag, tflag, vflag, wflag, xflag, nflag;
int	setnr;
char	**gargv;
int 	goptind, gargc;
...
char	*buffer;
{
...
#ifdef DEBUG
        printf("in process_file 1\n");
#endif
	s = (short *) buffer;
#ifdef DEBUG
        printf("in process_file 2\n");
#endif
	...
	for (i = 0; i < FANO; i++) {
#ifdef DEBUG
        printf("in process_file 3 %d %d\n",i, FANO);
	printf("%d\n",c);
	printf("%d\n",&buffer[c]);
#endif
		file_table[i] = (struct bsa *) &buffer[c];
#ifndef	SWAP
	printf("%d\n",file_table[i]->bsa_dol_w_size);
	printf("%d\n",file_table[i]->bsa_dol_w_type);
	printf("%s\n",file_table[i]->bsa_dol_t_text);
...
unsigned short	rsize;
{
	...
	while (file_count+i < filesize && i < rsize) {
		...
		default:
			fclose(f);
			unlink(filename);
/*	This fprintf was changed by De Clarke at Lick Observatory, to include */
/*	the filename of the suspect file.  It was kind of useless before.     */
			fprintf(stderr, 
			"Invalid record format = %d file %s\n", recfmt,filename);
			return;
		}
	}
	file_count += i;
...
/* This next case was added by De Clarke at Lick Observatory to handle the */
/* beginning of an image save set.  It seems to work now.		   */

		case brh_dol_k_volume:
#ifdef	DEBUG
			printf("record type is VOLUME\n");
#endif
			break;			/* end of D. Clarke addition */
...
#ifdef	DEBUG
			printf("rtype = file\n");
        printf("calling process_file\n");

#endif
			process_file(&block[i]);
#ifdef DEBUG
        printf("back process_file\n");
#endif
...
char	*argv[];
{
	...
	cflag=dflag=eflag=sflag=tflag=vflag=wflag=xflag=nflag=0;
	while((c=getopt(argc,argv,"cdef:s:tvwxn")) != EOF)
		switch(c){
		case 'c':
			cflag++;
			break;
