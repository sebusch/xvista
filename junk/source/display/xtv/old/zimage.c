...
/* Can we allocate enough colors? If not, use a private colormap */
  ...
  stcolor[3].green = 0;
  stcolor[3].blue = 0;
  XStoreColors(dpy, defcmap,stcolor,4);
  ...
  palimage = XCreateImage(dpy,DefaultVisual(dpy,screen),8,ZPixmap,0,palette,
  	       MAXPALWIDTH,1,8,0);
  /* Create the data XImage structure */
  dataimage = XCreateImage(dpy,DefaultVisual(dpy,screen),8,ZPixmap,0,imbuf,
  	       128,1024,8,0);
...
int flag;               /* 0/1 for image/vector color load */
{
  ...
  if(flag == 0) {
    ...
    XStoreColors(dpy, defcmap, vcolor, nvcolors);  
  }
  return (0);
}
...
int wid, hgt;
{
  ...
  /* xyz window width */
  xyzwidth = MIN(XYZWIDTH*fontwidth,width);
