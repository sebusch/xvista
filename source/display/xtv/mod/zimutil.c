...
replicate(x0,y0,w0,h0,from,f,x,y,w,h,wto,to,fill)
...
duplicate(x0,y0,w0,h0,from,x,y,w,h,wto,to,fill)
...
int fill;			/* flag to zero rest of "to" array */
{
  int i0,i1,j0,j1,j,k,l,ntot;
  j0 = MAX(0,y0);
  j1 = MIN(h0,y0+h);
  i0 = MAX(0,x0);
  i1 = MIN(w0,x0+w);
fprintf(stderr,"duplicate: %d %d %d %d %d\n",j0,j1,i0,i1,wto);
  ntot=0;
  for(j=j0; j<j1; j++) {
//    for (k=i0;k<i1;k++) {
//      for (l=0;l<3;l++) {
//      *(to + wto*(j-y0+y) + x + i0+(k-i0)*3-x0+l) = *(from + w0*j + k);
//   ntot++;
//      }
//    }
     bcopy(from + w0*j + i0,   to + wto*(j-y0+y) + x + i0-x0,   i1 - i0);
  }
