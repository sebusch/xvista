...
int *inst;              /* identifies instrument for resource/windowname */
{
  ...
  fprintf(stderr,"starting imageinit\n");
...
/* Can we allocate enough colors? If not, use a private colormap */
  ...
  if (truecolor) {
    ...
    if (XAllocColorCells(dpy, defcmap,
      ...
      if (XAllocColorCells(dpy, defcmap, False, &planes, 0, &pixels, *nvclrs+4+*nclrs) ==0 )
        fprintf(stderr,"Error in XAllocColorCells: %d\n",*nvclrs+4+*nclrs);

       for (i=0 ; i <*nvclrs+4+*nclrs; i++) {
  ...
  stcolor[3].green = 0;
  stcolor[3].blue = 0;
  status = XStoreColors(dpy, defcmap,stcolor,4);
  getcolors();
  ...
  if (depth>8) { 
   if((outbuf = (unsigned char *)malloc(3*MAXSCREENPIX))==NULL) {
    fprintf(stderr,"Can't allocate display buffer\n");
    return(ERR_BAD_ALLOC_IMBUF);
   }
  }
  ...
  palimage = XCreateImage(dpy,visual,depth,ZPixmap,0,palette,
  	       MAXPALWIDTH,1,8,0);
  fprintf(stderr,"created palimage\n");
  /* Create the data XImage structure */
  fprintf(stderr,"creating dataimage, depth: %d\n", depth);
  dataimage = XCreateImage(dpy,visual,depth,ZPixmap,0,imbuf,128,1024,8,0);
  if (truecolor||directcolor) {
    palimage->red_mask = rmask;
    palimage->green_mask = gmask;
    palimage->blue_mask = bmask;
  ...
  if(zoomf > 0) {
    ...
    zoomimage = XCreateImage(dpy,visual,8,ZPixmap,0,NULL,1,1,8,0);
  printf("created zoomimage\n");
  }
  ...
  *hinit = height; 
  ...
  fprintf(stderr,"done updateimage\n");

  tvinit = 1;
  fprintf(stderr,"done imageinit\n");
  return(0);
}
...
fprintf(stderr,"select input\n");
XSelectInput(dpy,wimage, ExposureMask);

/* Map the subwindows */
XMapSubwindows(dpy,wbase);
fprintf(stderr,"done map subwindows\n");
...
fprintf(stderr,"done createwindows\n");
return(0);
}
...
fprintf(stderr,"imageerase\n");
  ...
  XFlush(dpy);
fprintf(stderr,"done imageerase\n");
}
...
fprintf(stderr,"done imageupdate\n");
  return(0);
}
...
/* Otherwise damage the image window so as to get it displayed */
 ...
 XFlush(dpy);
 fprintf(stderr,"done imagedisplay\n");
 return (0);
}
...
{
  fprintf(stderr,"imagemap\n");
  palbrkpt = breakpts;
  brancher(ncolors,pixels,breakpts,lookup,branch);
  fprintf(stderr,"done imagemap\n");
  return(0);
}
...
int flag;               /* 0/1 for image/vector color load */
{
  ...
  fprintf(stderr,"done imagepalette\n");
  return (0);
}
...
{
fprintf(stderr,"imageinstallkey\n");
  keyaction[key].action = function;
  keyaction[key].echo = echo;
fprintf(stderr,"done imageinstallkey\n");
}
...
*/
    ...
    if (truecolor||directcolor) {
      dataimage->bits_per_pixel = depth;
...
/* Write the appropriately zoomed data into imbuf from image */
 ...
 //  if (depth==24) {
 //    triplicate(imbuf,outbuf,ROUNDUP(x1-x0)*(y1-y0));
 //    dataimage->data = (char *)outbuf;
 //  }

fprintf(stderr,"dataimage: %d %x %x %x\n",dataimage->bits_per_pixel,
   dataimage->red_mask,
   dataimage->green_mask,
   dataimage->blue_mask);
fprintf(stderr,"%d %d %d\n",
   *imbuf&dataimage->red_mask,
   *imbuf&dataimage->green_mask,
   *imbuf&dataimage->blue_mask);
    fprintf(stderr,"Xputimage\n");

    XPutImage(dpy,wimage,imagegc,  
	      dataimage,0,0,x0,y0,ROUNDUP(x1-x0),y1-y0); 
    fprintf(stderr,"done Xputimage\n");
/*
  }
*/
fprintf(stderr,"imagevreplay\n");
  imagevreplay();
fprintf(stderr,"imagetreplay\n");
  imagetreplay();
fprintf(stderr,"done writepix\n");
  XFlush(dpy);
...
getcolors()
{
  ...
  status = XQueryColors(dpy, defcmap,testcolor,4+nvcolors+ncolors);
  for (i=0; i<4+nvcolors+ncolors; i++) {
    fprintf(stderr,"%d %d %d %d %d\n",i,testcolor[i].pixel,
         testcolor[i].red, testcolor[i].green, testcolor[i].blue);
  }
}
triplicate(char *in,char *out, int n)
{
  int i;
