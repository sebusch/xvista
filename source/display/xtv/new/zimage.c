  ...
  if (nvisuals == 0) {
    ...
    *vinfo = XGetVisualInfo(dpy,VisualClassMask,&vtemplate,&nvisuals);
    if (nvisuals == 0) {
     ...
     printf("have a DirectColor mode %d!\n",nvisuals);
     directcolor = 1;
...
/* Can we allocate enough colors? If not, use a private colormap */
  ...
  if (truecolor) {
    ...
    if (XAllocColorCells(dpy, defcmap,
      ...
      defcmap = XCreateColormap(dpy,RootWindow(dpy,screen),visual,AllocAll );
      printf("defcmap returns: %d %d %d %d %d \n",defcmap, BadAlloc, BadMatch, BadValue, BadWindow);
      if (directcolor) 
        status = XAllocColorPlanes(dpy, defcmap, 0, pixels, *nvclrs+4+*nclrs,1,1,1,&rmask,&gmask,&bmask);
      else
        status = XAllocColorCells(dpy, defcmap, 0, &planes, 0, pixels, *nvclrs+4+*nclrs);
      printf("created colormap %d %d %d %d\n",status,*nvclrs,*nclrs,directcolor);
...
//	         pixels[i] = i; 
  ...
  printf("stored colors %d %d %d %d\n",status,BadAccess,BadColor,BadValue);
  ...
  if(zoomf > 0) {
    ...
    zoomimage = XCreateImage(dpy,visual,8,ZPixmap,0,NULL,
  	       1,1,8,0);
