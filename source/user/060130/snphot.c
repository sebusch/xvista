...
void snphot(PAR par[], int oldpar, FRAME frame[], int nframe, STAR star[], int nstar, 
            GAL gal[], int ngal, VAR *var, int nvar, int, int);
...
void dosnphot_(int *nim, float **locframe, int *nx, int *ny, double *crval1, double *crval2, double *cd1_1, double *cd1_2, double *cd2_1, double *cd2_2, int *crpix1, int *crpix2, int *cnpix1, int *cnpix2, int *tele, int *filt, double *back, double *jd, double *gain, double *rn, double *lowbad, double *highbad, float **locpsf, int *psfn, double *psfdx, int *niter, int *maxvar, int *maxgal, int *maxstar, float *dmax, int *backgrid, char *starfile, char *snfile, char *parfile, int ns, int nsn, int np)
{
...
#define MAXLINE 200
  ...
  *dmax /= 3600.;
  ...
  snphot(par,oldpar,frame,nframe,star,nstar,gal,ngal,var,nvar,*niter,*backgrid);
  ...
  strncpy(out,snfile,l);
  out[l] = 0;
  ifile = open(strcat(out,".par"),O_RDWR|O_CREAT);
  write(ifile,&par,MAXPAR*sizeof(PAR));
  close(ifile);
}

void snphot(PAR par[], int oldpar, FRAME frame[], int nframe, STAR star[], int nstar, 
            GAL gal[], int ngal, VAR var[], int nvar, int maxiter, int backgrid)
{
  int npar, redo=1, final=0, used, objpix, niter=0, ncombo, ipar, jpar, nfit, nskipped;
  ...
  float data;
  double mod, p, p2, tot, scale, dpdx, dpdy, diff, sig2, x, y, tmp;
  double trans;
        ...
        for (igx=0;igx<gal[igal].nsize;igx++) {
          for (igy=0;igy<gal[igal].nsize;igy++) {
            if (gal[igal].inten[igx][igy][tfilt]>0) {
              gal[igal].color[igx][igy] = gal[igal].inten[igx][igy][jfilt]/gal[igal].inten[igx][igy][tfilt];
      ...
      for (ix=0; ix<frame[iframe].nx; ix++) {
        ...
        for (iy=0; iy<frame[iframe].ny; iy++) {
         ...
         if (data>frame[iframe].lowbad&&data<frame[iframe].highbad) {
          ...
          mod = 0;
          dxdd = -cd1_2/den;
          dxdr = cd2_2/den*cos(crval2*D2R);
          dydd = cd1_1/den;
          dydr = -cd2_1/den*cos(crval2*D2R);
          ...
          for (igal=0; igal<ngal; igal++) {
            ...
            for (igx=0;igx<gal[igal].nsize;igx++) {
              gra=gal[igal].ra+(igx-gal[igal].nsize/2)*
                                gal[igal].dx/cos(frame[iframe].crval2*D2R);
              for (igy=0;igy<gal[igal].nsize;igy++) {
          ...
          if (niter==maxiter||final) 
            *(frame[iframe].data+iy*frame[iframe].nx+ix) = data-mod;
...
/* Correct parameters and check for convergence */
    ...
    niter++;
    if ((niter>maxiter)||final) redo=FALSE;
    if (converge) final = TRUE;
    fprintf(stderr,"niter: %d maxiter: %d redo:%d converge: %d final: %d\n",niter,maxiter,redo,converge,final);
  }
...
float update_inten(PAR *x)
{
  ...
  x->converge = 0;
  if (x->val>2*x->err) {
    if ( (x->val+x->del)/x->val > 2) {
      return (2*x->val);
    }
    if ( (x->val+x->del)/x->val < 0.5) {
      return (x->val/2.);
    }
...
void skytoxy(FRAME frame, double dec, double ra, double *x, double *y) 
{
  ...
  den=a*d-b*c;
  *x = frame.crpix1 + 
       d/den*(ra-frame.crval1)*cos(frame.crval2*D2R)-b/den*(dec-frame.crval2);
  *y = frame.crpix2 - 
       c/den*(ra-frame.crval1)*cos(frame.crval2*D2R)+a/den*(dec-frame.crval2);
}
void xytosky(FRAME frame, double x, double y, double *dec, double *ra)
{
  *ra = frame.crval1 + 
         ((x-frame.crpix1)*frame.cd1_1 + (y-frame.crpix2)*frame.cd1_2)/cos(frame.crval2*D2R);
  *dec = frame.crval2 + 
...
void dsolve(FRAME frame, double dec, double ra, double *dx, double *dy)
{
  ...
  dra=(ra-frame.crval1)*cos(frame.crval2*D2R);
  //inv[0][0] = d / den;
  //inv[0][1] = -b / den;
  //inv[1][0] = -c / den;
  //inv[1][1] = a / den;
...
int usepix(FRAME frame, double ra, double dec, double pra, double pdec, float fitrad) 
{
  ...
  dra=fabs(ra-pra)*cos(frame.crval2*D2R);
